...
# 如果handler函数需要传入命名关键字参数（KEYWORD_ONLY）而且参数没有默认值的话：
# 获取这些‘没有默认值的命名关键字参数’
...
class RequestHandler(object):
    ...
    async def __call__(self, request):
        kw = None
        logging.info(' %s : has_request_arg = %s, has_var_kw_arg = %s, has_named_kw_args = %s, get_named_kw_args = %s, get_required_kw_args = %s ' % (__name__, self._has_request_arg, self._has_var_kw_arg, self._has_name_kw_arg, self._named_kw_args, self._required_kw_args))
        # 如果handler需要传入命名关键字参数或者可变参数的话
        if self._has_var_kw_arg or self._has_name_kw_arg or self._required_kw_args:
            # 如果是post请求，则读请求的body
